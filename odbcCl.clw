  ...
  map 
    module('odbc32')
      SQLExecDirect(SQLHSTMT StatementHandle, *SQLCHAR StatementText, SQLINTEGER TextLength),sqlReturn,pascal,raw
      ...
      SQLPrepare(SQLHSTMT StatementHandle, *SQLCHAR StatementText, SQLINTEGER TextLength),sqlReturn,pascal,raw
...
retv    sqlReturn,auto
  ...
  retv = SQLExecDirect(self.conn.gethStmt(), sqlCode.Cstr(), SQL_NTS)
...
!  retv = SQLPrepare(self.conn.gethStmt(), self.sqlStr.cstr(), SQL_NTS)
  !stop(retv)
  retv = SQLExecDirect(self.conn.gethStmt(), self.sqlStr.cstr(), self.sqlStr.strlen())
