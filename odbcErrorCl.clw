  ...
  map 
    module('odbc32')
      ...
      SQLGetDiagRec(SQLSMALLINT HandleType, SQLHANDLE Handle, SQLSMALLINT RecNumber, odbcWideStr errState, *SQLINTEGER NativeErrorPtr, odbcWideStr MessageText, SQLINTEGER BufferLength, *SQLINTEGER TextLengthPtr),sqlReturn,pascal,raw,name('SQLGetDiagRecW')
    end 
  end
...
tempholder bool
  ...
  loop count = 1 to self.errorCount
    ...
    retv = SQLGetDiagRec(handleType, handle, count, stateMsg.getWideStr(), self.errorMsgQ.NativeErrorPtr, errMsg.GetWideStr(), size(self.errorMsgQ.MessageText), self.errorMsgQ.textLengthPtr)
